...
SELECT IF(german_votes>italian_votes,"Yes","No") AS Answer
FROM votes_summary;
...
GROUP BY  name
ORDER BY  COUNT(rol.movie_id) DESC LIMIT 2;
...
with genre_sum AS 
    ...
    FROM genre
    GROUP BY  genre
    ORDER BY  count(movie_id) DESC limit 3), movie_sum AS 
    (SELECT gs.genre,
		year,
        title AS movie_name,
        cast((replace(replace(worlwide_gross_income,'INR',''),'$','')) AS decimal(10)) AS worldwide_gross_income, 
        rank() over(partition by year ORDER BY  cast(replace(replace(worlwide_gross_income,'INR',''),'$','') AS decimal(10)) desc) AS movie_rank
    FROM movie AS m
    INNER JOIN genre AS g
        ON m.id=g.movie_id
    INNER JOIN genre_sum AS gs
...
SELECT production_company,
        COUNT(m.id) AS movie_count,
		RANK() OVER(ORDER BY COUNT(m.id) DESC) AS prod_comprank
FROM movie AS m
INNER JOIN ratings AS r
    ON m.id= r.movie_id
...
AS (SELECT
  id,
  ROUND (AVG (DATEDIFF (next_date, date_published)), 2) AS avg_date_diff
FROM (SELECT
  n.id,
  m.title,
  m.date_published,
  LEAD(m.date_published, 1) OVER (PARTITION BY n.id ORDER BY date_published) AS next_date
FROM director_mapping d
INNER JOIN movie m
  ON d.movie_id = m.id
INNER JOIN names n
  ON n.id = d.name_id) AS dates
GROUP BY id),
director_summary
AS (SELECT
  ...
  dd.avg_date_diff AS avg_inter_movie_days,
  AVG(r.avg_rating) AS avg_rating,
  SUM(r.total_votes) AS total_votes,
  MIN(r.avg_rating) AS min_rating,
  MAX(r.avg_rating) AS max_rating,
  SUM(m.duration) AS total_duration,
  ROW_NUMBER() OVER (ORDER BY COUNT(d.movie_id) DESC) AS rownumber
FROM director_mapping d
INNER JOIN names n
  ON n.id = d.name_id
INNER JOIN director_dates AS dd
  ON n.id = dd.id
